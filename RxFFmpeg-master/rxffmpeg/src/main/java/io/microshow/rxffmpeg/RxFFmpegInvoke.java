...
public class RxFFmpegInvoke {
     ...
     * 异步执行
     * 报错:
     * "A/libc: Fatal signal 11 (SIGSEGV), code 128 (SI_KERNEL), fault addr 0x0 in tid 10577
     * (.bilibili.merge), pid 10577 (.bilibili.merge)"
     *
     * @param command
     * @param mffmpegListener
     */
    @Deprecated // use: runCommandAsync(final String[] command, final ProcessingComplete processingComplete)
    public void runCommandAsync(final String[] command, IFFmpegListener mffmpegListener) {
        ...
        synchronized (RxFFmpegInvoke.class) {
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    int ret = runFFmpegCmd(command);// 0就是成功
                    onClean();
                }
            }).start();
        }
     ...
     * (.bilibili.merge), pid 10577 (.bilibili.merge)"
     *
     * @param command       需要异步执行的ffmpeg命令
     * @param processingComplete    但执行完成之后的操作
     */
    public void runCommandAsync(final String[] command, final ProcessingComplete processingComplete) {
        setFFmpegListener(null);
        synchronized (RxFFmpegInvoke.class) {
            // 不允许多线程访问
            // 这种方式不行:
            new Thread(new Runnable() {
